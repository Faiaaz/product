...
function webpaceMapsLoadMarkerMap(id, hue, saturation, lightness, gamma, zoom, type, bike, traffic, transit) {
    var data = {
        ...
        if (response.success) {
            ...
            for (var i = 0; i < maps.length; i++) {
                ...
                google.maps.event.addListener(autocomplete_marker, 'place_changed', function () {
                    ...
                    geocoder.geocode({'address': addr}, function (results, status) {
                        if (newmarker) {
                            ...
                            webpaceMapsPlaceMarker(results[0].geometry.location)
                        }
                        map.setCenter(results[0].geometry.location);
                        webpaceMapsUpdateMarkerInputs(results[0].geometry.location);
                    });
                });
                ...
                google.maps.event.addListener(map, 'rightclick', function (event) {
                    webpaceMapsPlaceMarker(event.latLng);
                    webpaceMapsUpdateMarkerInputs(event.latLng);
                });
                ...
                jQuery("#marker_location_lat #marker_location_lng").on("change", function () {
                    ...
                    webpaceMapsPlaceMarker(position);
                    ...
                    for (var y = 0; y < markers.length; y++) {
                        ...
                        if (markerid == id) {
                            ...
                            google.maps.event.addListener(map_marker_edit, 'rightclick', function (event) {
                                ...
                                webpaceMapsUpdateMarkerEditInputs(event.latLng);
                            });
                            ...
                            google.maps.event.addListener(autocomplete_edit_marker, 'place_changed', function () {
                                ...
                                geocoder.geocode({'address': addr}, function (results, status) {
                                    ...
                                    webpaceMapsUpdateMarkerEditInputs(results[0].geometry.location);
                                })
                            })

                            webpaceMapsUpdateMarkerEditInputs(markeredit.getPosition());
                        }
                    }
                    google.maps.event.trigger(map_marker_edit, 'resize');
...
function webpaceMapsPlaceMarker(location) {
    if (newmarker) {
...
function webpaceMapsUpdateMarkerInputs(location) {
...
function webpaceMapsUpdateMarkerEditInputs(location) {
