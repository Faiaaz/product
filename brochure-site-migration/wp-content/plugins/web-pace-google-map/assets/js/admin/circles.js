...
function webpaceMapsLoadCircleMap(id, hue, saturation, lightness, gamma, zoom, type, bike, traffic, transit) {
    data = {
        ...
        if (response.success) {
            ...
            for (var i = 0; i < maps.length; i++) {
                ...
                google.maps.event.addListener(autocomplete_circle, 'place_changed', function () {
                    ...
                    geocoder.geocode({'address': addr}, function (results, status) {
                        if (newcircle) {
                            ...
                            webpaceMapsPlaceCircle(results[0].geometry.location)
                        }
                        mapcircle.setCenter(results[0].geometry.location);
                        webpaceMapsUpdateCircleInputs(results[0].geometry.location);
                    })
                });
                ...
                google.maps.event.addListener(mapcircle, 'rightclick', function (event) {
                    webpaceMapsPlaceCircle(event.latLng);
                    webpaceMapsUpdateCircleInputs(event.latLng);
                });
                        ...
                        if (circleid == id) {
                            ...
                            google.maps.event.addListener(circlemarkeredit, "drag", function (event) {
                                if (editcircle) {
                                    ...
                                    webpaceMapsUpdateCircleEditInputs(event.latLng);
                                }
                            });
                            ...
                            webpaceMapsUpdateCircleEditInputs(circlemarkeredit.getPosition());
...
function webpaceMapsPlaceCircle(location) {
    if (circlemarker) {
    ...
    google.maps.event.addListener(circlemarker, "drag", function (event) {
        ...
        webpaceMapsUpdateCircleInputs(circlemarker.getPosition());
    })
...
function webpaceMapsUpdateCircleInputs(location) {
...
function webpaceMapsUpdateCircleEditInputs(location) {
