jQuery(document).ready(function () {
    jQuery('#webpacemapinsert').on('click', function () {
        var id = jQuery('#webpace_map_select option:selected').val();

        window.send_to_editor('[webpace_maps id="' + id + '"]');
        tb_remove();
        ...
        id = jQuery('#webpace_map_select option:selected').val();
        var data = {
            action: "webpace_maps_save_shortcode_options",
    ...
    jQuery("#webpace_map_select").on("change", function () {
        var name = jQuery("#map_name").val();
        var type = jQuery("#map_type").val();
        var width = jQuery("#map_width").val();
        var height = jQuery("#map_height").val();
        ...
        id = jQuery('#webpace_map_select option:selected').val();
        var data = {
            action: "webpace_maps_shortcode_change_map",
            nonce: inlinePopupL10n.nonce,
            map_id: id,
            name: name,
            type: type,
