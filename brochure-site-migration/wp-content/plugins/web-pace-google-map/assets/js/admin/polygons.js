...
function webpaceMapsLoadPolygonMap(id, hue, saturation, lightness, gamma, zoom, type, bike, traffic, transit) {
    data = {
    ...
    jQuery.ajax({
        ...
        webpaceMapsInitializePolygonMap(response);
    }).fail(function () {
        console.log('Failed to load response from database');
    });
    function webpaceMapsInitializePolygonMap(response) {
        if (response.success) {
            ...
            for (var i = 0; i < maps.length; i++) {
                ...
                google.maps.event.addListener(mappolygone, 'rightclick', function (event) {
                    webpaceMapsPlacePolygone(event.latLng);
                    webpaceMapsUpdatePolygoneInputs(event.latLng);
                });
                        ...
                        if (polygoneid == id) {
                            ...
                            for (var j = 0; j < latlngs.length; j++) {
                                ...
                                google.maps.event.addListener(polygoneditmarker[j], 'click', function (event) {
                                    ...
                                    this.setMap(null);
                                    webpaceMapsUpdatePolygoneEditInputs();
...
function webpaceMapsUpdatePolygoneInputs(location) {
    var temp_array = "";
...
function webpaceMapsUpdatePolygoneEditInputs() {
    var temp_array = "";
...
function webpaceMapsPlacePolygone(location) {
    ...
    google.maps.event.addListener(polygonmarker[array_index], 'click', function (event) {
        ...
        this.setMap(null);
        webpaceMapsUpdatePolygoneInputs();
    ...
    google.maps.event.addListener(polygonmarker[array_index], "drag", function (e) {
        ...
        newpolygon.setPaths(newpolygoncoords);
        webpaceMapsUpdatePolygoneInputs(position);
    });
    var polygone_line_color = "#" + jQuery('#polygone_line_color').val();
    var polygone_line_opacity = jQuery('#polygone_line_opacity').val();
    var polygone_fill_color = "#" + jQuery('#polygone_fill_color').val();
