...
function webpaceMapsLoadLocatorsMap(id, hue, saturation, lightness, gamma, zoom, type, bike, traffic, transit) {
    data = {
    ...
    jQuery.ajax({
        ...
        webpaceMapsInitializeLocatorMap(response);
    }).fail(function () {
        console.log('Failed to load response from database');
    });

    function webpaceMapsInitializeLocatorMap(response){
        if( !response.success ){
            return false;
        }
