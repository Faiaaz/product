...
function webpaceMapsLoadPolylineMap(id, hue, saturation, lightness, gamma, zoom, type, bike, traffic, transit) {
    data = {
    ...
    jQuery.ajax({
        ...
        webpaceMapsInitializePolylineMap(response);
    }).fail(function () {
        console.log('Failed to load response from database');
    });
    function webpaceMapsInitializePolylineMap(response) {
        if (response.success) {
            ...
            for (var i = 0; i < maps.length; i++) {
                ...
                google.maps.event.addListener(mappolyline, 'rightclick', function (event) {
                    webpaceMapsPlacePolyline(event.latLng);
                    webpaceMapsUpdatePolylineInputs(event.latLng);
                });
                        ...
                        if (polylineid == id) {
                            ...
                            for (var j = 0; j < latlngs.length; j++) {
                                ...
                                google.maps.event.addListener(polylineeditmarker[j], 'click', function (event) {
                                    ...
                                    this.setMap(null);
                                    webpaceMapsUpdatePolylineEditInputs();
...
function webpaceMapsUpdatePolylineInputs(location) {
    var temp_array = "";
...
function webpaceMapsUpdatePolylineEditInputs() {
    var temp_array = "";
...
function webpaceMapsPlacePolyline(location) {
    ...
    google.maps.event.addListener(polylinemarker[array_index], 'click', function (event) {
        ...
        this.setMap(null);
        webpaceMapsUpdatePolylineInputs();
    ...
    google.maps.event.addListener(polylinemarker[array_index], "drag", function (e) {
        ...
        newpolyline.setPath(newpolylinecoords);
        webpaceMapsUpdatePolylineInputs(position);
    });
    var polyline_line_color = "#" + jQuery('#polyline_line_color').val();
    var polyline_line_opacity = jQuery('#polyline_line_opacity').val();
    var polyline_line_width = jQuery('#polyline_line_width').val();
