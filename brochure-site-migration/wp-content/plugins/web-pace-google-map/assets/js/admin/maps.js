function webpaceMapsInitializeAllMaps(id, response){
    webpaceMapsLoadMap(id, "#" + response.hue, response.saturation, response.lightness, response.gamma, response.zoom, response.type, response.bike, response.traffic, response.transit, response.animation, response.draggable, response.wheel_scroll);
    webpaceMapsLoadMarkerMap(id, "#" + response.hue, response.saturation, response.lightness, response.gamma, response.zoom, response.type, response.bike, response.traffic, response.transit);
    webpaceMapsLoadPolygonMap(id, "#" + response.hue, response.saturation, response.lightness, response.gamma, response.zoom, response.type, response.bike, response.traffic, response.transit);
    webpaceMapsLoadPolylineMap(id, "#" + response.hue, response.saturation, response.lightness, response.gamma, response.zoom, response.type, response.bike, response.traffic, response.transit);
    webpaceMapsLoadCircleMap(id, "#" + response.hue, response.saturation, response.lightness, response.gamma, response.zoom, response.type, response.bike, response.traffic, response.transit);
    webpaceMapsLoadLocatorsMap(id, "#" + response.hue, response.saturation, response.lightness, response.gamma, response.zoom, response.type, response.bike, response.traffic, response.transit);
}
...
function webpaceMapsDeleteItem(id, table, li, x) {
    var delete_data = {
        action: 'webpace_maps_delete_item',
...
jQuery(document).ready(function () {
    ...
    jQuery("#map_name_tab").on("keyup change", function () {
        ...
        var data = {
            action: 'webpace_maps_change_map_name',
    ...
    jQuery(".admin_edit_section_container").on("click",".edit_list_delete_submit", function () {
        ...
        switch(type){
            case "direction":
                ...
                webpaceMapsDeleteItem(id, table, li, x);
...
function webpaceMapsLoadMap(id, hue, saturation, lightness, gamma, zoom, type, bike, traffic, transit, animation , draggable, wheel_scroll) {

    data = {
        action: 'webpace_maps_get_info',
        map_id: id
    };
    jQuery.ajax({
        url: ajaxurl,
        ...
        if (response.success) {
            ...
            for (var i = 0; i < maps.length; i++) {
                ...
                for (j = 0; j < markers.length; j++) {
                    ...
                    WebpaceMapsBindInfoWindow(marker[j], map_admin_view, infowindow, description, info_type);
                }
            }
...
function WebpaceMapsBindInfoWindow(marker, map, infowindow, description, info_type) {
...
function webpaceMapsAttachInstructionText(stepDisplay, marker, text, map) {
    google.maps.event.addListener(marker, 'click', function() {
