...
function webpaceMapsBindInfoWindow(marker, map, infowindow, description, infoType, openOnload) {
    if (openOnload) {
...
jQuery(document).ready(function () {
    function webpaceMapsInitializeMap(elementId) {
        ...
        function webpaceMapsFrontRefresh() {
            google.maps.event.trigger(front_end_map, 'resize');
        }

        jQuery(document).on('click',function () {
            setTimeout(function () {
                webpaceMapsFrontRefresh();
            }, 500);
        })
        ...
        jQuery.ajax({
            ...
            webpaceMapsInitializeMap(response);
            locStores = response.success.locators;
        }).fail(function () {
            console.log('Failed to load response from database');
        });
        function webpaceMapsInitializeMap(response) {
            if (response.success) {
                ...
                for (var i = 0; i < markers.length; i++) {
                    ...
                    webpaceMapsBindInfoWindow(marker[i], front_end_map, currentInfoWindow, description, dataInfoType, dataOpenInfowindowsOnload);
                }
                    ...
                    geocoder.geocode({'address': locAddress}, function (result, status) {
                        if (status == google.maps.GeocoderStatus.OK) {
                            ...
                            fromLatLng = new google.maps.LatLng(result[0].geometry.location.lat(), result[0].geometry.location.lng());
                                if(!!oldLatLng){
                                    if(oldLatLng.lat()===fromLatLng.lat() && oldLatLng.lng()===fromLatLng.lng() && webpaceTheSame ){
                                        return false;
                                    }
                                }
                            ...
                            getFinalStores(locStores).then(function () {
                                if (finalStores.length > 0) {
                                    ...
                                    labelIndex = 1;
                                    jQuery(document).find("#webpc_main_store_block").remove();
                                    jQuery(".webpace_google_map_container").after("<div id='webpc_main_store_block'></div>");
                                    var StoreBlock = "";
                                    ...
                                    for (var i in finalStores) {
                                        ...
                                        jQuery("#webpc_main_store_block").append(StoreBlock);
                                        ...
                                        jQuery(document).find("#webpc_main_store_block .str-name").last().text(finalStores[i].name);
                                        jQuery(document).find("#webpc_main_store_block .str-addr").last().text(finalStores[i].locator_addr);
                                        jQuery(document).find("#webpc_main_store_block .str-number").last().text(finalStores[i].locator_phone);
                                        jQuery(document).find("#webpc_main_store_block .str-dir a").last().prop('href', 'https://www.google.com/maps/dir//' + gDirQuery);
                                        jQuery(document).find("#webpc_main_store_block .str-img img").last().before("<p class='markerLabel'>" + locMarker.label + "</p>");
                                        jQuery(document).find("#webpc_main_store_block .a_info").last().find("table tr td:last-child").each(function () {
    ...
    var allMaps = jQuery('.webpace_google_map');
    ...
    if (allMaps.length) {
        ...
        allMaps.each(function (i) {
            ...
            webpaceMaps[i] = webpaceMapsInitializeMap(id);
