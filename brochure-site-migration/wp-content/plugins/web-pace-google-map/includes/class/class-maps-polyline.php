...
/**
 * Class Webpace_Maps_Polyline
 */
class Webpace_Maps_Polyline implements Webpace_Maps_Polyline_Interface {
	...
	/**
	 * Webpace_Maps_Polyline constructor.
	 *
	 * @param null $id
	 *
	 * @throws Error
	...
	public function __construct( $id = null ) {
		...
		if ( $id !== null && absint( $id ) == $id ) {
			...
			$polyline = $wpdb->get_row( $wpdb->prepare(
				"
                SELECT *
                FROM " . Webpace_Maps()->get_table_name( 'polylines' ) . "
                WHERE id=%d
                ",
				$id
			), ARRAY_A );
			...
			$this->name = __( 'New Polyline', 'webpace_map' );
		}
	...
	public static function delete( $id ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM " . Webpace_Maps()->get_table_name( 'polylines' ) . " WHERE id =%d", $id ) );
	}
	...
	public function save() {
		...
		$success = is_null( $this->id )
			? $wpdb->insert( Webpace_Maps()->get_table_name( 'polylines' ), $polygon_data )
			: $wpdb->update( Webpace_Maps()->get_table_name( 'polylines' ), $polygon_data, array( 'id' => $this->id ) );
