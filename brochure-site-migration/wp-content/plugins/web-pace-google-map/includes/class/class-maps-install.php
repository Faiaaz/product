...
class Webpace_Maps_Install {
	...
	/**
	 * Check Webpace_Maps version and run the updater if required.
	 *
	 * This check is done on all requests and runs if the versions do not match.
	 */
	public static function check_version() {
		if ( get_option( 'webpace_maps_version' ) !== Webpace_Maps()->version ) {
			self::install();
			update_option( 'webpace_maps_version', Webpace_Maps()->version );
			do_action( 'webpace_maps_version' );
		}
	}
	...
	public static function install() {
		...
		wp_mkdir_p( 'webpace-google-map-custom-icons' );
        ...
        $new_columns=array(
            Webpace_Maps()->get_table_name( 'maps' )=>array(
                'locator_enabled'=>'INT DEFAULT 0',
                'locator_default_address'=> 'text DEFAULT NULL',
            ),
            Webpace_Maps()->get_table_name( 'stores' )=>array(
	...
	private static function create_tables() {
		...
		if ( ! self::$needs_refactoring ) {
			...
			$webpace_maps_maps_exists = $wpdb->get_row( "SELECT * FROM information_schema.tables WHERE table_schema = '" . DB_NAME . "' AND table_name = '" . $wpdb->prefix . "webpace_maps_maps' LIMIT 1;", ARRAY_A );

			self::$create_default_rows_and_set_options = empty( $webpace_maps_maps_exists );
		...
		$wpdb->query(
			"CREATE TABLE IF NOT EXISTS " . $wpdb->prefix . "webpace_maps_maps(
		...
		$wpdb->query(
			"CREATE TABLE IF NOT EXISTS " . $wpdb->prefix . "webpace_maps_markers(
                id int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
		...
		$wpdb->query(
			"CREATE TABLE IF NOT EXISTS " . $wpdb->prefix . "webpace_maps_polygons(
                id int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
		...
		$wpdb->query(
			"CREATE TABLE IF NOT EXISTS " . $wpdb->prefix . "webpace_maps_polylines(
                id int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
                map_id int(11) UNSIGNED NOT NULL,
                name text DEFAULT NULL,
		...
		$wpdb->query(
			"CREATE TABLE IF NOT EXISTS " . $wpdb->prefix . "webpace_maps_circles(
                id int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
		...
		$wpdb->query(
			"CREATE TABLE IF NOT EXISTS " . $wpdb->prefix . "webpace_maps_directions(
                id int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
                map_id int(11) DEFAULT NULL,
                name text DEFAULT NULL,
        ...
        $wpdb->query(
            "CREATE TABLE IF NOT EXISTS " . $wpdb->prefix . "webpace_maps_stores(
                id int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
                map_id int(11) UNSIGNED DEFAULT NULL,
	...
	private static function alter_tables(){
        global $wpdb;
        $maps_table = Webpace_Maps()->get_table_name( 'maps' );
	...
	private static function insert_default_map(){
        $map = new Webpace_Maps_Map();

        $map
            ->set_name( __( 'My First Map', 'webpace-maps' ) )
            ->set_info_type( 'click' )
            ->set_type( 'ROADMAP' )
            ->set_zoom( 7 )
    ...
    private static function insert_default_markers( $map ){
        $marker1 = new Webpace_Maps_Marker();

        $marker1
            ->set_map_id( $map )
            ->set_name( __('New York','webpace_map') )
            ->set_lat( 40.7127837 )
            ->set_lng( -74.00594130000002 )
            ->set_animation( 'BOUNCE' )
            ->set_description( __( 'New York City', 'webpace_map' ) );

        $marker1->save();


        $marker2 = new Webpace_Maps_Marker();

        $marker2
            ->set_map_id( $map )
            ->set_name( __( 'Delaver', 'webpace_map' ) )
            ->set_lat( 39.189690821096804 )
            ->set_lng( -75.7562255859375 )
            ->set_animation( 'DROP' )
            ->set_description( __( 'Delaver', 'webpace_map' ) );
    ...
    private static function insert_default_polygon( $map ){
        ...
        $polygon
            ->set_map_id($map)
            ->set_url( 'http://www.webpace.com' )
            ->set_name( __( 'My First Polygon', 'webpace_map' ) )
    ...
    private static function insert_default_polyline( $map ){
        ...
        $polyline
            ->set_map_id( $map )
            ->set_name( __( 'My First Polyline', 'webpace_map' ) )
    ...
    private static function insert_default_circle( $map ){
        ...
        $circle
            ->set_map_id( $map )
            ->set_name( __( 'My First Circle', 'webpace_map' ) )
