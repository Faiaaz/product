...
/**
 * Class Webpace_Maps_Polygon
 */
class Webpace_Maps_Polygon implements Webpace_Maps_Polygon_Interface {
	...
	/**
	 * Webpace_Maps_Polygon constructor.
	 *
	 * @param null $id
	 *
	 * @throws Error
	...
	public function __construct( $id = null ) {
		...
		if ( $id !== null && absint( $id ) == $id ) {
			...
			$polygon = $wpdb->get_row( $wpdb->prepare(
				"
                SELECT *
                FROM " . Webpace_Maps()->get_table_name( 'polygons' ) . "
                WHERE id=%d
                ",
				$id
			), ARRAY_A );
			...
			$this->name = __( 'New Polygon', 'webpace_map' );
	...
	public static function delete( $id ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM " . Webpace_Maps()->get_table_name( 'polygons' ) . " WHERE id =%d", $id ) );
	}
	...
	public function save() {
		...
		$success = is_null( $this->id )
			? $wpdb->insert( Webpace_Maps()->get_table_name( 'polygons' ), $polyline_data )
			: $wpdb->update( Webpace_Maps()->get_table_name( 'polygons' ), $polyline_data, array( 'id' => $this->id ) );
