...
/**
 * Class Webpace_Maps_Map
 */
class Webpace_Maps_Map implements Webpace_Maps_Map_Interface {
	 ...
	 * Array of Webpace_Maps_Marker instances
	 *
	 * @var array
	 */
	private $markers;
     ...
     * Array of Webpace_Maps_Locator instances
     * @var array
     */
    private $locators;
	 ...
	 * Array of Webpace_Maps_Polygon instances
	 * @var array
	 */
	private $polygons;
	 ...
	 * Array of Webpace_Maps_Polyline instances
	 * @var array
	 */
	private $polylines;
	 ...
	 * Array of Webpace_Maps_Circle instances
	 * @var array
	 */
	private $circles;
	 ...
	 * Array of Webpace_Maps_Direction instances
	 * @var array
	 */
	private $directions;

	/**
	 * Webpace_Maps_Map constructor.
	 *
	 * @param null|int $id
	 *
	 * @throws Error
	...
	public function __construct( $id = null ) {
		...
		if ( $id !== null && absint( $id ) == $id ) {
			...
			$map = $wpdb->get_row( $wpdb->prepare(
				"
                SELECT *
                FROM " . Webpace_Maps()->get_table_name( 'maps' ) . "
                WHERE id=%d
                ",
				$id
			), ARRAY_A );
			...
			$this->markers    = Webpace_Maps_Query::get_markers( array( 'map_id' => $this->id ) );
			$this->polygons   = Webpace_Maps_Query::get_polygons( array( 'map_id' => $this->id ) );
			$this->polylines  = Webpace_Maps_Query::get_polylines( array( 'map_id' => $this->id ) );
			$this->circles    = Webpace_Maps_Query::get_circles( array( 'map_id' => $this->id ) );
			$this->directions = Webpace_Maps_Query::get_directions( array( 'map_id' => $this->id ) );
            $this->locators   = Webpace_Maps_Query::get_locator( array( 'map_id' => $this->id ) );

		} else {

			$this->name = __( 'New Map', 'webpace_map' );
	...
	public function get_name() {
		return (!empty($this->name) ? wp_unslash($this->name) : __( '(no title)', 'webpace_map' ) );
	}

	/**
	 * @param string $name
	 *
	 * @return Webpace_Maps_Map
	 */
	...
	/**
	 * @param Webpace_Maps_Marker[] $markers
	 *
	 * @return Webpace_Maps_Map
	 * @throws Exception
	 */
	public function set_markers( $markers ) {
		foreach ( $markers as $marker ) {
			...
			if ( ! ( $marker instanceof Webpace_Maps_Marker ) ) {
				throw new Exception( 'Marker must be an instance of Webpace_Maps_Marker class.' );
			}
	...
	/**
	 * @param Webpace_Maps_Polygon[] $polygons
	 *
	 * @return Webpace_Maps_Map
	 * @throws Exception
	 */
	public function set_polygons( $polygons ) {
		foreach ( $polygons as $polygon ) {
			...
			if ( ! ( $polygon instanceof Webpace_Maps_Polygon ) ) {
				throw new Exception( 'Polygon must be an instance of Webpace_Maps_Polygon class.' );
			}
	...
	/**
	 * @param Webpace_Maps_Polyline[] $polylines
	 *
	 * @return Webpace_Maps_Map
	 * @throws Exception
	 */
	public function set_polylines( $polylines ) {
		foreach ( $polylines as $polyline ) {
			...
			if ( ! ( $polyline instanceof Webpace_Maps_Polyline ) ) {
				throw new Exception( 'Polyline must be an instance of Webpace_Maps_Polyline class.' );
			}
    ...
    /**
     * @param Webpace_Maps_Locator[] $locators
     *
     * @return Webpace_Maps_Map
     * @throws Exception
     */
    public function set_locators( $locators ) {
        foreach ( $locators as $locator ) {
            ...
            if ( ! ( $locator instanceof Webpace_Maps_Locator ) ) {
                throw new Exception( 'Locator must be an instance of Webpace_Maps_Locator class.' );
            }
	...
	/**
	 * @param Webpace_Maps_Circle[] $circles
	 *
	 * @return Webpace_Maps_Map
	 * @throws Exception
	 */
	public function set_circles( $circles ) {
		foreach ( $circles as $circle ) {
			...
			if ( ! ( $circle instanceof Webpace_Maps_Circle ) ) {
				throw new Exception( 'Circle must be an instance of Webpace_Maps_Circle class.' );
			}
	...
	/**
	 * @param Webpace_Maps_Direction[] $directions
	 *
	 * @return Webpace_Maps_Map
	 * @throws Exception
	 */
	public function set_directions( $directions ) {
		foreach ( $directions as $direction ) {
			...
			if ( ! ( $direction instanceof Webpace_Maps_Direction ) ) {
				throw new Exception( 'Direction must be an instance of Webpace_Maps_Direction class.' );
			}
	...
	public function get_edit_link() {
		...
		$link = admin_url( 'admin.php?page=webpace_maps&task=edit_map&id=' . $this->id );

		$link = wp_nonce_url( $link, 'webpace_maps_edit_map_' . $this->id );
	...
	public static function delete( $id ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM " . Webpace_Maps()->get_table_name( 'maps' ) . " WHERE id =%d", $id ) );
	}
	...
	public function save($map_id=null) {
        ...
        $map_success = is_null( $this->id )
            ? $wpdb->insert( Webpace_Maps()->get_table_name( 'maps' ), $map_data )
            : $wpdb->update( Webpace_Maps()->get_table_name( 'maps' ), $map_data, array( 'id' => $this->id ) );
