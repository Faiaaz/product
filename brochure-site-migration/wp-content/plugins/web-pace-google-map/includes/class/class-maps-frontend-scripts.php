...
/**
 * Class Webpace_Maps_Frontend_Scripts
 */
class Webpace_Maps_Frontend_Scripts {
	...
	public static function init() {
		add_action( 'webpace_maps_shortcode_scripts', array( __CLASS__, 'add_scripts' ) );
		add_action( 'webpace_maps_shortcode_scripts', array( __CLASS__, 'add_styles' ) );
	...
	public static function add_scripts( $map_id ) {
		...
		wp_enqueue_script( 'webpace-google-maps-api', 'https://maps.googleapis.com/maps/api/js?' . $key_param . 'libraries=places,geometry', false, null, true );

		wp_enqueue_script( 'webpace_maps_frontend', Webpace_Maps()->plugin_url() . '/assets/js/frontend/frontend.js', array(
			'webpace-google-maps-api',
			'jquery'
		), false, true );

		wp_localize_script( 'webpace_maps_frontend', 'mapFrontL10n', array(
        ...
        wp_localize_script( 'webpace_maps_frontend', 'frontdir_options', array(
	...
	public static function add_styles( $map_id ) {
		wp_enqueue_style( 'webpace_maps_frontend', Webpace_Maps()->plugin_url() . '/assets/css/frontend.css' );
        wp_enqueue_style( 'webpace_maps_store_locator', Webpace_Maps()->plugin_url() . '/assets/css/store-locator.css' );
	}
}