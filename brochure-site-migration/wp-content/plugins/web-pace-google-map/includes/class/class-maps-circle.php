...
/**
 * Class Webpace_Maps_Circle
 */
class Webpace_Maps_Circle implements Webpace_Maps_Circle_Interface {
	...
	/**
	 * Webpace_Maps_Circle constructor.
	 *
	 * @param null $id
	 *
	 * @throws Error
	...
	public function __construct( $id = null ) {
		...
		if ( $id !== null && absint( $id ) == $id ) {
			...
			$circle = $wpdb->get_row( $wpdb->prepare(
				"
                SELECT *
                FROM " . Webpace_Maps()->get_table_name( 'circles' ) . "
                WHERE id=%d
                ",
				$id
			), ARRAY_A );
			...
			$this->name = __( 'New Circle', 'webpace-maps' );
	...
	public static function delete( $id ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM " . Webpace_Maps()->get_table_name( 'circles' ) . " WHERE id =%d", $id ) );
	}
	...
	public function save() {
		...
		$success = is_null( $this->id )
			? $wpdb->insert( Webpace_Maps()->get_table_name( 'circles' ), $circle_data )
			: $wpdb->update( Webpace_Maps()->get_table_name( 'circles' ), $circle_data, array( 'id' => $this->id ) );
