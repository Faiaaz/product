...
/**
 * Class Webpace_Maps_Locator
 */
class Webpace_Maps_Locator implements Webpace_Maps_Locator_Interface {
	...
	/**
	 * Webpace_Maps_Locator constructor.
	 *
	 * @param null $id
	 *
	 * @throws Error
	...
	public function __construct( $id = null ) {
		...
		if ( $id !== null && absint( $id ) == $id ) {
			...
			$locator = $wpdb->get_row( $wpdb->prepare(
				"
                SELECT *
                FROM " . Webpace_Maps()->get_table_name( 'stores' ) . "
                WHERE id=%d
                ",
				$id
			), ARRAY_A );
			...
			$this->name = __( 'New Store Locator', 'webpace_map' );
	...
	public static function delete( $id ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM " . Webpace_Maps()->get_table_name( 'stores' ) . " WHERE id =%d", $id ) );
	}
	...
	public function save() {
		...
		$success = is_null( $this->id )
			? $wpdb->insert( Webpace_Maps()->get_table_name( 'stores' ), $locator_data )
			: $wpdb->update( Webpace_Maps()->get_table_name( 'stores' ), $locator_data, array( 'id' => $this->id ) );
