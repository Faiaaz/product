...
/**
 * Class Webpace_Maps_Marker
 */
class Webpace_Maps_Marker implements Webpace_Maps_Marker_Interface {
	...
	/**
	 * Webpace_Maps_Marker constructor.
	 *
	 * @param null $id
	 *
	 * @throws Error
	...
	public function __construct( $id = null ) {
		...
		if ( $id !== null && absint( $id ) == $id ) {
			...
			$marker = $wpdb->get_row( $wpdb->prepare(
				"
                SELECT *
                FROM " . Webpace_Maps()->get_table_name( 'markers' ) . "
                WHERE id=%d
                ",
				$id
			), ARRAY_A );
			...
			$this->name = __( 'New Marker', 'webpace_map' );
		}
	...
	public static function get_default_icon_sizes() {
		return apply_filters( 'webpace_maps_default_marker_icon_sizes', array( 16, 24, 48, 64, 96, 128, 192, 256 ) );
	}
	...
	public static function get_default_icons() {
		return apply_filters( 'webpace_maps_default_marker_icons',
			array(
				'azuremarker',
				'baseflag',
				'blackmarker',
	...
	public static function delete( $id ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM " . Webpace_Maps()->get_table_name( 'markers' ) . " WHERE id =%d", $id ) );
	}
	...
	public function save() {
		...
		$success = is_null( $this->id )
			? $wpdb->insert( Webpace_Maps()->get_table_name( 'markers' ), $marker_data )
			: $wpdb->update( Webpace_Maps()->get_table_name( 'markers' ), $marker_data, array( 'id' => $this->id ) );
